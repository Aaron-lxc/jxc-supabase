...
window.U = {
  ...
  /* 状态排序权重：启用类档案（已启用/启用 在前；未启用 次之；已停用 最末）
     用于商品/客户/供应商/仓库/合伙人/收货方/成员/佣金规则等带「启用」开关的列表 */
  rankEnabled(s) {
    s = '' + (s == null ? '' : s);
    if (s === '已启用' || s === '启用') return 0;
    if (s === '已停用') return 2;
    if (s === '未启用') return 1;
    return 0; // 无状态默认在前
  },

  /* 状态排序权重：流程类单据（待办优先——未生效/未完成/未确认/未计算/未处理/未发放 在前；
     已生效/已完成/已确认/已计算/已处理/已发放/已预存 在后；已作废/已停用 最末）
     用于调拨/生产/销售/收入/支出/活动/投诉/奖励等带流程状态的列表 */
  rankTodo(s) {
    s = '' + (s == null ? '' : s);
    if (s === '已作废' || s === '已停用') return 2;
    if (/^未/.test(s)) return 0;
    return 1;
  }
};
