...
ok(om.find(m => m.key === 'report') && !om.find(m => m.key === 'report').children,
  'owner 的运营报表应为叶子菜单（无子菜单）');
...
ok(mm.length === 13, 'member 默认权限下菜单应为 13 项（含活动管理，无财务/设置/账户管理）');
...
ok(mm.find(m => m.key === 'report') && !mm.find(m => m.key === 'report').children,
  'member 默认应看到运营报表（无子菜单）');
